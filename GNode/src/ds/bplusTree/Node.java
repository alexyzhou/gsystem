...
public class Node<T extends Comparable<T>, V> implements Serializable { 
    ...
    /** 叶节点的前节点*/ 
    protected Node<T, V> previous; 
     
    /** 叶节点的后节点*/ 
    protected Node<T, V> next;     
     
    /** 节点的关键字 */ 
    protected List<Entry<T, V>> entries; 
     
    /** 子节点 */ 
    protected List<Node<T, V>> children; 
     
    public Node(boolean isLeaf) { 
        this.isLeaf = isLeaf; 
        entries = new ArrayList<Entry<T, V>>(); 
         
        if (!isLeaf) { 
            children = new ArrayList<Node<T, V>>(); 
        } 
    } 
    ...
    public void insertOrUpdate(T key, V obj, BPlusTree<T,V> tree){ 
        //如果是叶子节点 
        if (isLeaf){ 
            //不需要分裂，直接插入或更新 
            ...
            //需要分裂   
                ...
                //分裂成左右两个节点 
            	Node<T, V> left = new Node<T, V>(true); 
            	Node<T, V> right = new Node<T, V>(true); 
                //设置链接 
                if (previous != null){ 
                ...
                //如果是根节点     
                    ...
                    isRoot = false; 
                    Node<T, V> parent = new Node<T, V>(false, true); 
                    tree.setRoot(parent); 
                    left.setParent(parent); 
                    right.setParent(parent); 
    ...
    /** 插入节点后中间节点的更新 */ 
    protected void updateInsert(BPlusTree<T, V> tree){ 
        ...
        if (children.size() > tree.getOrder()) { 
            //分裂成左右两个节点 
            Node<T, V> left = new Node<T, V>(false); 
            Node<T, V> right = new Node<T, V>(false); 
            //左右两个节点关键字长度 
            int leftSize = (tree.getOrder() + 1) / 2 + (tree.getOrder() + 1) % 2; 
            int rightSize = (tree.getOrder() + 1) / 2; 
            ...
            for (int i = 0; i < leftSize; i++){ 
                ...
                left.getEntries().add(new SimpleEntry<T, V>(children.get(i).getEntries().get(0).getKey(), null)); 
                children.get(i).setParent(left); 
            } 
            for (int i = 0; i < rightSize; i++){ 
                right.getChildren().add(children.get(leftSize + i)); 
                right.getEntries().add(new SimpleEntry<T, V>(children.get(leftSize + i).getEntries().get(0).getKey(), null)); 
    ...
    /** 调整节点关键字*/ 
    protected void validate(Node<T, V> node, BPlusTree<T, V> tree) { 
         
        // 如果关键字个数与子节点个数相同 
        if (node.getEntries().size() == node.getChildren().size()) { 
            for (int i = 0; i < node.getEntries().size(); i++) { 
                T key = node.getChildren().get(i).getEntries().get(0).getKey(); 
                if (node.getEntries().get(i).getKey().compareTo(key) != 0) { 
                    node.getEntries().remove(i); 
                    node.getEntries().add(i, new SimpleEntry<T, V>(key, null)); 
                    if(!node.isRoot()){ 
            ...
            for (int i = 0; i < node.getChildren().size(); i++) { 
                T key = node.getChildren().get(i).getEntries().get(0).getKey(); 
                node.getEntries().add(new SimpleEntry<T, V>(key, null)); 
                if (!node.isRoot()) { 
                    validate(node.getParent(), tree); 
                } 
            } 
    ...
    /** 删除节点后中间节点的更新*/ 
    protected void updateRemove(BPlusTree<T, V> tree) { 
        ...
        if (children.size() < tree.getOrder() / 2 || children.size() < 2) { 
            if (isRoot) { 
                ...
                Node<T, V> previous = null, next = null; 
                if (prevIdx >= 0) { 
                ...
                if (previous != null  
                    ...
                    Node<T, V> borrow = previous.getChildren().get(idx); 
                ...
                // 如果后节点子节点数大于M / 2并且大于2，则从其处借补 
                    ...
                    //后叶子节点首位添加到末尾 
                	Node<T, V> borrow = next.getChildren().get(0); 
                ...
                // 否则需要合并节点 
                    ...
                    if (previous != null  
                        ...
                        for (int i = previous.getChildren().size() - 1; i >= 0; i--) { 
                        	Node<T, V> child = previous.getChildren().get(i); 
                    ...
                    // 同后面节点合并 
                        ...
                        for (int i = 0; i < next.getChildren().size(); i++) { 
                        	Node<T, V> child = next.getChildren().get(i); 
                            children.add(child); 
                            child.setParent(this); 
                        } 
    ...
    public void remove(T key, BPlusTree<T, V> tree){ 
        //如果是叶子节点 
        if (isLeaf){ 
            ...
            if (isRoot) { 
                ...
                if (entries.size() > tree.getOrder() / 2 && entries.size() > 2) { 
                    ...
                    if (previous != null  
                        ...
                        Entry<T, V> entry = previous.getEntries().get(size - 1); 
                    ...
                    //如果自身关键字数小于M / 2，并且后节点关键字数大于M / 2，则从其处借补    
                        ...
                        Entry<T, V> entry = next.getEntries().get(0); 
        ...
        //如果不是叶子节点   
            ...
            //如果 key小于等于节点最左边的key，沿第一个子节点继续搜索 
            if (key.compareTo(entries.get(0).getKey()) <= 0) { 
    ...
    /** 插入到当前节点的关键字中*/ 
    protected void insertOrUpdate(T key, V obj){ 
        Entry<T, V> entry = new SimpleEntry<T, V>(key, obj); 
        //如果关键字列表长度为 0，则直接插入 
        if (entries.size() == 0) { 
            entries.add(entry); 
            return; 
    ...
    public void setPrevious(Node<T, V> previous) { 
    ...
    public List<Entry<T, V>> getEntries() { 
        return entries; 
    } 
 
    public void setEntries(List<Entry<T, V>> entries) { 
    ...
    public void setChildren(List<Node<T, V>> children) { 
